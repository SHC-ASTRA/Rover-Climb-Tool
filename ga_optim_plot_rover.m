function [state,options,optchanged] = ga_optim_plot_rover(options,state,flag,wheel_radius)
if flag == 'iter'
    ...
    for i = 1:pop_size(1)
        ...
        plot(wheel_f_x_list,wheel_f_y_list,'Color',[0, 1, 1, 0.05]);
        plot(wheel_r_x_list,wheel_r_y_list,'Color',[0, 1, 1, 0.05]);
        plot(suspension_x_list,suspension_y_list,'Color',[0, 1, 1, 0.05]);
    end
    
    [~,min_idx] = min(state.Score);
    x = state.Population(min_idx,:);
    wheelbase_length = x(1);
    suspension_height = x(2);
    suspension_trap_len = x(3);
    suspension_trap_offs=x(4);
    ...
    plot(suspension_x_list,suspension_y_list,"b");
